...
class ControllerMailOrder extends Controller {
    ...
    public function add($order_info, $order_status_id, $comment, $notify) {
        ...
        if (isset($order_info['shipping_code']) && $order_info['shipping_code'] == 'collector.collector') {
            $data['shipping_method'] = \Agmedia\Kaonekad\Models\ShippingCollector::getTitle($order_info['shipping_collector_id']);
        }
        ...
        foreach ($order_products as $order_product) {
            ...
            $data['products'][] = array(
                ...
                'priceeur'    => $this->currency->format($order_product['price'] + ($this->config->get('config_tax') ? $order_product['tax'] : 0), 'EUR'),
                'total'    => $this->currency->format($order_product['total'] + ($this->config->get('config_tax') ? ($order_product['tax'] * $order_product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
                'totaleur'    => $this->currency->format($order_product['total'] + ($this->config->get('config_tax') ? ($order_product['tax'] * $order_product['quantity']) : 0), 'EUR')
            );
        }
        ...
        foreach ($order_totals as $order_total) {
            $data['totals'][] = array(
                ...
                'text'  => '<small>'.$this->currency->format($order_total['value'], 'EUR'). '</small> '.$this->currency->format($order_total['value'], $order_info['currency_code'], $order_info['currency_value']),
            );
        }
