...
class ModelExtensionReportSale extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT MIN(o.collect_date) AS date_start, MAX(o.collect_date) AS date_end, COUNT(*) AS `orders`, o.payment_code, SUM((SELECT SUM(op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id)) AS products, SUM((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(o.collect_date) <= DATE('" . $this->db->escape($data['filter_date_end']) . "')";
		}
		...
		switch($group) {
			case 'day';
				$sql .= " GROUP BY YEAR(o.collect_date), MONTH(o.collect_date), DAY(o.collect_date)";
				break;
			default:
			case 'week':
				$sql .= " GROUP BY YEAR(o.collect_date), WEEK(o.collect_date)";
				break;
			case 'month':
				$sql .= " GROUP BY YEAR(o.collect_date), MONTH(o.collect_date)";
				break;
			case 'year':
				$sql .= " GROUP BY YEAR(o.collect_date)";
				break;
		}

		$sql .= " ORDER BY o.collect_date DESC";
	...
	public function getTotalOrders($data = array()) {
		...
		switch($group) {
			case 'day';
				$sql = "SELECT COUNT(DISTINCT YEAR(collect_date), MONTH(collect_date), DAY(collect_date)) AS total FROM `" . DB_PREFIX . "order`";
				break;
			default:
			case 'week':
				$sql = "SELECT COUNT(DISTINCT YEAR(collect_date), WEEK(collect_date)) AS total FROM `" . DB_PREFIX . "order`";
				break;
			case 'month':
				$sql = "SELECT COUNT(DISTINCT YEAR(collect_date), MONTH(collect_date)) AS total FROM `" . DB_PREFIX . "order`";
				break;
			case 'year':
				$sql = "SELECT COUNT(DISTINCT YEAR(collect_date)) AS total FROM `" . DB_PREFIX . "order`";
				break;
		}
        ...
        if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(collect_date) >= DATE('" . $this->db->escape($data['filter_date_start']) . "')";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(collect_date) <= DATE('" . $this->db->escape($data['filter_date_end']) . "')";
		}
